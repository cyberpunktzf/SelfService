...
function LoadSelect(){
    ...
    var inputtradetype = {};
    inputtradetype['TradeCode'] = "queryTradeType^"  + "SelfServPy.Common.mysql_contral" + "^" + "mysqlsql";
    CallMethod(inputtradetype,initSelectTradetype,"DoMethod");

}

function initSelectTradetype(jsonObj) {
    var tmpArr = [];
...
function Clear(){
    selectRow = null;
    $('.layui-form').find('input').not('#QueryBtn').not('#ClearBtn').not('#ss_td_update').val('');
    layui.use('form', function() {
        var form = layui.form;
        var formTable = $('.layui-form').not('#ss_td_update').find('select');
        ...
        layui.use('table', function(){
            ...
            table.on('row(dg)', function(obj){
                if(selectRow && selectRow.data.id == obj.data.id){ // 取消选中
                    ...
                    var formTable = $('.layui-form').find('input').not('#QueryBtn').not('#ss_td_update').not('#ClearBtn');
