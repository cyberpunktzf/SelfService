 ...
 */
var GLOBAL = {
    MODULENAME : 'SelfServPy.Common.ss_eqmfaultCtl',
    CLASSNAME :'EMFC',
    FILEDS:{
        '1':{'title':'设备编号','id':'ss_eqmf_eqcode','seq':'1'},
        '2':{'title':'设备名称','id':'ss_eqmf_eqdesc','seq':'2'},
        '3':{'title':'模块编号','id':'ss_eqmf_modcode','seq':'3'},
        '4':{'title':'模块名称','id':'ss_eqmf_moddesc','seq':'4'},
        '5':{'title':'模块状态','id':'ss_eqmf_modstatus','seq':'5'},
        '6':{'title':'故障代码','id':'ss_eqmf_faultcode','seq':'6'},
        '7':{'title':'故障描述','id':'ss_eqmf_faultdesc','seq':'7'}
    }
}
var selectRow;
$(function () {
...
function initSelect(jsonObj) {
    ...
    layui.use(['form'], function() {
        ...
        $.each(tmpArr,function(index,value){
            $('#ss_eqmf_eqcode').append(new Option(value.ss_eqlistd_eqcode))
        });
        form.render("select");
        form.on('select(ss_eqmf_eqcode)',function(data){
            var input = {};
            input['TradeCode'] = "query^"  + "SelfServPy.Common.ss_eqlistdCtl" + "^" + "ELC";
            ...
            CallMethod(input,function(jsonObj){
                ...
                $('#ss_eqmf_eqdesc').val(tmpArr[0].ss_eqlistd_eqdesc);
            },"DoMethod");
        });
