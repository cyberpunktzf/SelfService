...
class DHC:
    ...
    def Init(self,Input,ClsHisOPObj):
        ...
        # desc
        desc = ""     
        InsertDic = {'terminal_info':terminal_info,'serial_number':serial_number,'code':code,'desc':desc,'processcode':processcode,'usercode' : UserCode}
        BMObj = BMCtl.BM()
    ...
    def DHC_3300(self,Input,ClsHisOPObj):
        ...
        if ClsHisOPObj.patinfo_dict.get('readcard_type'): 
            if ClsHisOPObj.patinfo_dict.get('readcard_type') != "": 
                barCode = ""
        ...
        code_val = Response
        his_master_id = ""
        if Tools.GetXMLNode(Response,'Response'):
            his_master_id = Tools.GetXMLNode(Response,'PatientID')
        InsertDic = {'fk_businessmaster_id':fk_businessmaster_id,'code':code,'code_val':code_val,'his_master_id':his_master_id}
        PTObj.insert(InsertDic)
        # 构造返回参数
        Response = Tools.BuildWebOutPut(Response)
    ...
    def DHC_PayServ_InsuOPDividePre(self,Input,ClsHisOPObj):
        ...
        AdmReasonId = '2'
        #取HIS返回的
        BDOutPut = Tools.GeBDOutPut(BusinessMasterId,intef_code = '4904')
        InsType = Tools.GetXMLNode(BDOutPut,'InsType')
        if InsType :
            if InsType !="":
                AdmReasonId = InsType
        #取选择的
    ...
    def DHC_PayServ_InsuOPDivideStrike(self,Input,ClsHisOPObj):
        ...
        # dhandle, AdmInfoDr, UserID, AdmReasonId, ExpStr
        InsuDivDr = ""
        INSURtn = Tools.GeBDOutPut(BusinessMasterId,'InsuOPDivideRollBackBack','SaveBD')
        #解析医保串  医保患者
        if INSURtn !="" :
            InsuDivDr = INSURtn.split('^')[1] #0^150^200^0^0^门大(城职)联网已结算^^CZZG^1!1^306^033^046^048^047^049^050^0!N
        #取费别
    ...
    def DHC_GetDepInfoById(self,DepId,ClsHisOPObj):
        ...
        Response = CallHISWS(RequestStr)
        return Response
    def BuildCardStrBak (self,ClsHisOPObj):
        INSUCardStr = ClsHisOPObj.patinfo_dict.get('insu_card_no') 
        ...
        MtType = MTLB
        Password = '111111'
        CardStr = MedCardNo # + '|' + Password + '|' + PersonalID + '|' + CheckCode + '|' + MultitudeCategory + '|' + InsurTypeCode + '|' + MtType
        return CardStr
    ...
    def DHC_4902(self,Input,ClsHisOPObj):
        ...
        if processcode == "NAReg-Reg" :
            ...
            tmpinsertInput = {
                "code":"Charge",
                "id":int(BusinessMasterId)
            }
            BMCtl1.insert(tmpinsertInput)
        PTObj = PTCtl.PT()
    ...
    def DHC_SaveBD(self,Input,ClsHisOPObj):
        ...
        if tmpmodal_code:
            if tmpmodal_code == "INSUPreRegBack" and tmpOutput == "":
                raise Exception("6969^医保网络异常请重试一次，谢谢")
        BDObj = BDCtl.BD()
    ...
    # 返回格式
    # 患者自负金额^医保账户支付金额$医保支付方式1!医保支付方式1金额$医保支付方式2!医保支付方式2金额……
    # 取患者最终自负金额(微信支付宝)
    ...
    def PayServInvocieBill(self,Input,ClsHisOPObj):
        try:
            ...
            BDOutPut = Tools.GeBDOutPut(BusinessMasterId,intef_code = '4905')
            if BDOutPut:
                HISPrtRowID = Tools.GetXMLNode(BDOutPut,'InvoiceNo')
            if HISPrtRowID == "":
                BDOutPut = Tools.GeBDOutPut(BusinessMasterId,intef_code = '1101')
                if  BDOutPut:
                    HISPrtRowID = Tools.GetXMLNode(BDOutPut,'HisInvprtId')
            #  
            OrgHISPrtRowID = ""
            PathCode = ""
            ExpStr = ""
    ...
    def GetCertPrint(self,Input,ClsHisOPObj):
        try:
            ...
            PatientID = ClsHisOPObj.his_patinfo_dict.get('his_unique_no') 
            AdmId = ""
            if ClsHisOPObj.patinfo_dict.get('barcode'):
